...
public class GameStartBlock extends Block {
    ...
    protected InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos,
        ...
        if (manager.isCountdownActive()) {
            player.sendSystemMessage(Component.translatable("message.huntersmarket.countdown_in_progress"));
        } else if (manager.getState() == GameState.IN_PROGRESS) {
            player.sendSystemMessage(Component.translatable("message.huntersmarket.already_in_progress"));
        } else if (manager.getState() == GameState.ENDED) {
            player.sendSystemMessage(Component.translatable("message.huntersmarket.game_ended_reset_required"));
        }
