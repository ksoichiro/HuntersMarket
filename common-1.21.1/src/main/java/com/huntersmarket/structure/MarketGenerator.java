package com.huntersmarket.structure;
...
public class MarketGenerator {
    private static final ResourceLocation TEMPLATE_ID =
            ResourceLocation.fromNamespaceAndPath(HuntersMarket.MOD_ID, "market");
    ...
    public static boolean generate(ServerLevel level, BlockPos center) {
        ...
        if (template.isEmpty()) {
            HuntersMarket.LOGGER.warn("Market template not found: {}", TEMPLATE_ID);
            return false;
        }
        ...
        HuntersMarket.LOGGER.info("Market placed at {}, spawn set to {}", placePos, spawnPos);
        return true;
    }
