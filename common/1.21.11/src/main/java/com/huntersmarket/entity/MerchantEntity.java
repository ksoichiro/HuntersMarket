...
import com.huntersmarket.trade.PriceList;
...
public class MerchantEntity extends Mob {
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!manager.canSell()) {
            ((ServerPlayer) player).sendSystemMessage(Component.translatable("message.huntersmarket.game_not_started"));
        ...
        if (heldItem.isEmpty() || !PriceList.isSellable(heldItem.getItem())) {
            ((ServerPlayer) player).sendSystemMessage(Component.translatable("message.huntersmarket.not_sellable"));
            setUnhappyCounter(20);
            this.playSound(SoundEvents.VILLAGER_NO, 1.0f, 1.0f);
            return InteractionResult.CONSUME;
        }
        ...
        serverPlayer.connection.send(new ClientboundSoundPacket(
                net.minecraft.core.Holder.direct(SoundEvents.PLAYER_LEVELUP), SoundSource.PLAYERS,
