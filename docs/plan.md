# Hunter's Market Implementation Plan

This document outlines the phased implementation plan for the Hunter's Market Minecraft mod based on [draft.md](draft.md).

## Package Structure

- Base package: `com.huntersmarket`
- Sub-packages:
  - `registry` - Item, Block, Entity, Structure registration
  - `state` - Game state management
  - `entity` - Merchant entity
  - `block` - Game Start Block, Game Reset Block
  - `item` - Custom items (reserved for future use)
...
- [x] 1-1. Create common mod initializer `HuntersMarket` in `common-shared`
  - Mod ID constant: `huntersmarket`
...
No custom items needed for Hunter's Market. Players receive vanilla Iron Sword, Shield, and Bread.

### Phase 4: Custom Blocks

Implement game control blocks.

- [x] 4-1. **Game Start Block** (`huntersmarket:game_start_block`)
...
- [x] 4-2. **Game Reset Block** (`huntersmarket:game_reset_block`)
...
- [x] 7-2. **Sales Amount Display**
  ...
  - Create gold coin texture: `assets/huntersmarket/textures/gui/coin.png`
...
- [x] 8-1. Design market structure using structure block / NBT
  ...
  - Save as `data/huntersmarket/structure/market.nbt`
- [x] 8-2. Create structure set and template pool
  - `data/huntersmarket/worldgen/structure/market.json`
  - ~~`data/huntersmarket/worldgen/structure_set/market.json`~~ (Not needed: MarketGenerator places the structure directly at spawn, so structure_set is unused)
  - `data/huntersmarket/worldgen/template_pool/market.json`
...
- [x] 8-4. Support manual placement via `/place structure huntersmarket:market`
...
- [x] 10-1. **Initial Equipment on Spawn**
  - Grant Iron Sword, Shield on first spawn and respawn (death)
...
- [x] 11-1. Create `PriceEventManager`
...
- [x] 11-3. Sync current prices to clients for HUD/UI updates
- [x] 11-4. Restore original prices when event ends
...
3. **Phase 3** - Custom items (none needed for Hunter's Market)
...
```
common-shared/src/main/java/com/huntersmarket/
├── HuntersMarket.java              # Common mod initializer
├── registry/
│   ├── ModItems.java
│   ├── ModBlocks.java
│   ├── ModEntityTypes.java
...
fabric-base/src/main/java/com/huntersmarket/fabric/
├── HuntersMarketFabric.java        # Fabric entry point
└── client/
    └── HuntersMarketFabricClient.java
...
neoforge-base/src/main/java/com/huntersmarket/neoforge/
├── HuntersMarketNeoForge.java      # NeoForge entry point
└── client/
    └── HuntersMarketNeoForgeClient.java
